...
function buildWakeText(payload: WakePayload, paperclipEnv: Record<string, string>): string {
  ...
  const lines = [
    ...
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$PAPERCLIP_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"testing\",\"rework\",\"merging\",\"blocked\"]}",
    "   - Treat the returned issue as the active issue for this run. A blocked issue may redirect you to an actionable blocker.",
    "   - GET /api/issues/{activeIssueId}",
    "   - GET /api/issues/{activeIssueId}/comments",
    "   - Execute the issue instructions exactly.",
    "   - If instructions require a comment, POST /api/issues/{activeIssueId}/comments with {\"body\":\"...\"}.",
    "   - PATCH /api/issues/{activeIssueId} with {\"status\":\"done\",\"comment\":\"what changed and why\"}.",
    "4) If issueId does not exist:",
    "   - GET /api/companies/$PAPERCLIP_COMPANY_ID/issues?assigneeAgentId=$PAPERCLIP_AGENT_ID&status=todo,in_progress,testing,rework,merging,blocked",
    "   - Pick in_progress first, then testing / rework / merging, then todo, then blocked, then execute step 3 using the returned active issue id.",
    "",
