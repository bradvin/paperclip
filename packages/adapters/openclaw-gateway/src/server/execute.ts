...
function buildWakeText(payload: WakePayload, paperclipEnv: Record<string, string>): string {
  ...
  const lines = [
    ...
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$PAPERCLIP_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"rework\",\"merging\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$PAPERCLIP_COMPANY_ID/issues?assigneeAgentId=$PAPERCLIP_AGENT_ID&status=todo,in_progress,rework,merging,blocked",
    "   - Pick in_progress first, then rework / merging, then todo, then blocked, then execute step 3 using the returned active issue id.",
    "",
